...
def execute(filters=None):
	...
	for sle in sl_entries:
		...
		#	0, sle.in_qty, sle.qty_after_transaction,
		#	sle.valuation_rate, sle.stock_value, sle.voucher_type, 
		#	sle.voucher_no, sle.serial_no, 
		#	])
		...
		#	item_detail.item_group,
		#	item_detail.brand, item_detail.description, sle.warehouse,
		#	sle.in_qty, sle.out_qty, sle.qty_after_transaction,
		#	sle.valuation_rate, sle.stock_value, sle.voucher_type, 
		#	sle.voucher_no, sle.serial_no, 
		#	])
		data.append([sle.date, sle.item_code, 
			item_detail.item_group,
			item_detail.brand, item_detail.description, sle.warehouse,
			sle.actual_qty if sle.voucher_type == "Purchase Invoice" else 0, 
			sle.actual_qty if sle.voucher_type == "Sales Invoice" else 0, 
...
def get_columns():
	columns = [
		...
		{"label": _("In Qty"), "fieldname": "in_qty", "fieldtype": "Float", "width": 50, "convertible": "qty"},
		{"label": _("Out Qty"), "fieldname": "out_qty", "fieldtype": "Float", "width": 50, "convertible": "qty"},
...
def get_stock_ledger_entries(filters, items):
	...
	#case when ( voucher_type = 'Purchase Invoice' and actual_qty >= 1 ) then actual_qty end as in_qty,
	#case when ( voucher_type = 'Purchase Invoice' and actual_qty <= -1 ) then actual_qty end as in_qty,
	#case when ( voucher_type = 'Sales Invoice' and actual_qty <= -1 ) then actual_qty end as out_qty,
	#case when ( voucher_type = 'Sales Invoice' and actual_qty >= 1 ) then actual_qty end as out_qty,
